...
model_mixture_joint_distribution <- function(data, selected_variables, model_type, copula = "NO") {
  ...
  if (model_type == "normal" && copula == "NO") {
    ...
    fig_3d <- plot_ly(density_data, x = ~Continuous_Var, y = ~Discrete_Var, z = ~Density, 
                       type = 'scatter3d', mode = 'lines', 
                       color = ~Discrete_Var, colors = color_palette, line = list(width = 4)) %>%
              layout(scene = list(xaxis = list(title = continuous_vars),
                                  yaxis = list(title = discrete_vars),
                                  zaxis = list(title = 'Hustota')))
  ...
  # Modelovanie hustoty pouzitim jadroveho vyhladzovania a kopule (rozklad na marginaly)
  ...
  calculate_density_3D <- function(data, category) {
    sub_data <- filter(data, .data[[discrete_vars]] == category)
    if (nrow(sub_data) > 1) {
      bw <- bw.nrd0(sub_data[[continuous_vars]])  # Optimalizovaná šírka pásma
      kde_x <- density(sub_data[[continuous_vars]], bw = bw)
      kde_x_fun <- approxfun(kde_x$x, kde_x$y, rule = 2)  # Aproximácia pre interpoláciu
  ...
  data <- data %>%
    mutate(U = ecdf(.data[[continuous_vars]])(.data[[continuous_vars]]),  # Empiricka distribućna funkcia
           V = as.numeric(as.factor(.data[[discrete_vars]])) / length(categories))  # Normalizovana diskretna premenna
...
result <- model_mixture_joint_distribution(mtcars, selected_variables, "kernel", "YES")
print(result)
```
