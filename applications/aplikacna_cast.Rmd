...
# Modelovanie podmienenej hustoty 2D
```{r}
plot_conditional_continuous_densities <- function(df, n_breaks = 5, density_scaling = 2000, mean_curve = TRUE, quantiles = NULL, mean_poly_degree = 1, quantile_poly_degree = 1, normal_density = TRUE, empirical_density = TRUE) {
...
plot_conditional_discrete_densities <- function(df, n_breaks = 5, density_scaling = 2000) {
  ...
  density_data <- do.call(rbind, lapply(split(df, df$section), function(sub_df) {
    ...
    y_vals <- as.numeric(levels(sub_df$response))
...
plot_conditional_densities <- function(data, selected_variables, n_breaks = 5, density_scaling = 2000, mean_curve = TRUE, quantiles = NULL, mean_poly_degree = 1, quantile_poly_degree = 1, normal_density = TRUE, empirical_density = TRUE) {
  ...
  if (response_var %in% var_types$Diskretne) {
    ...
    plot_conditional_discrete_densities(df, n_breaks, density_scaling)
    
  } else if (response_var %in% var_types$Spojite) {
    
    message(paste("Odozva", response_var, "bola identifikovana ako spojita."))
    plot_conditional_continuous_densities(df, n_breaks, density_scaling, mean_curve, quantiles, mean_poly_degree, quantile_poly_degree, normal_density, empirical_density)
...
```{r}
printVariables(mtcars)
result <- combine_conditional_models(data = mtcars, c("hp", "mpg"), "spline", "poly", NULL, 2, c(0.05, 0.95))
...
```{r}
printVariables(mtcars)
plot_conditional_densities(data = mtcars, selected_variables = c("hp", "carb"), n_breaks = 8, density_scaling = 50, mean_curve = FALSE, quantiles = NULL, mean_poly_degree = 2, quantile_poly_degree = 3, normal_density = TRUE, empirical_density = TRUE)
```
...
Vyriesene:
  ...
  - modelovanie podmienenej hustoty pravdepodobnosti
