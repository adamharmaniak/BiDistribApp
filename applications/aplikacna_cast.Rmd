...
plot_classification_ridge_1D <- function(data, response_name, predictor_name, model, method) {
  ...
  if (method == "logistic") {
    ...
    if (length(levels(response)) == 2) {
      ...
      pred <- ifelse(probs > 0.5, levels(response)[2], levels(response)[1])
    } else {
    ...
    pred <- class::knn(
      train = model$train_data,
      test = data[, predictor_name, drop = FALSE],
      cl = model$train_response,
      k = model$k
    )
  } else {
  ...
  data$pred_class <- factor(pred, levels = classes)
  class_var <- "pred_class"

  # Iba prítomné triedy v predikcii
  present_levels <- levels(droplevels(data[[class_var]]))
  present_indices <- match(present_levels, classes)
  colors_used <- my_colors[present_indices]

  p <- ggplot(data, aes_string(x = predictor_name, y = class_var, fill = class_var)) +
    geom_density_ridges(alpha = 0.7, scale = 1.2, color = "white") +
    scale_fill_manual(values = setNames(colors_used, present_levels)) +
    labs(
      title = paste("Ridge Plot -", method, "(Predikcia modelu)"),
...
classification_model <- function(data, response_name, predictor_names, method = "logistic", k = NULL, ridge = FALSE) {
...
model_joint_distribution_density(data = mtcars, selected_variables = c("hp", "mpg"), model_type = "kernel", bw = NULL, use_copula = FALSE, copula_type = NULL, marginal_density = NULL)
```

```{r}
printVariables(mtcars)
result <- classification_model(data = mtcars, response_name = "carb", predictor_names = c("hp"), method = "logistic", k = NULL, ridge = FALSE)
