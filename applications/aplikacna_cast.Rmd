...
model_joint_distribution <- function(data, variables) {
  ...
  if(length(discrete_vars) + length(continuous_vars) > 2){
    stop("Počet premenných musí byť presne 2.")
  }
...
model_multi_joint_distribution <- function(data, selected_variables) {
  # Identifikácia diskrétnych a spojitých premenných
  discrete_vars <- selected_variables[sapply(data[, selected_variables, drop = FALSE], 
                                             function(col) is.factor(col) || is.character(col) || 
                                                           (is.numeric(col) && length(unique(col)) < 10))]
  continuous_vars <- setdiff(selected_variables, discrete_vars)
  
  # Ak počet premenných presahuje 2 => vytvorenie komplexnej tabuľky pravdepodobnosti
  if (length(discrete_vars) + length(continuous_vars) > 2) {
    # Vytvorenie tabuľky s frekvenciami pre diskrétne premenné
    tab <- as.data.frame(table(data[, discrete_vars]))
    ...
    stop("Počet premenných musí byť väčší ako 2 na vytvorenie komplexnej tabuľky pravdepodobnosti.")
  }
}
```
