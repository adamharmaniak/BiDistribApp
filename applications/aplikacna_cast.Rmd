...
```{r, warning=FALSE}
mixture_joint_distribution <- function(data, discrete_vars, continuous_vars, model_type) {
...
```{r, warning=FALSE}
model_joint_distribution_density <- function(data, selected_variables, model_type = NULL, copula = FALSE, copula_type = NULL) {
  ...
  if (length(selected_variables) == 2 && length(continuous_vars) == 0) {
    ...
    result <- mixture_joint_distribution(data, discrete_vars, continuous_vars, model_type)
    print(result)
  }
}
```
...
```{r, warning=FALSE}
model_conditional_mean_value <- function(data, response_name, predictor_name, quantiles = 0.5, specific_x = NULL, mean_method = "loess", poly_degree = NULL) {
  
  if (mean_method == "poly" && is.null(poly_degree)) {
    stop("Pri pouzivani metody 'poly' je potrebne zadat stupen polynomu!")
  }
  
  if (mean_method != "poly" && !is.null(poly_degree)) {
    stop("Stupen polynomu sa zadava iba pri metode 'poly'!")
  }
  ...
  if (mean_method == "loess") {
    ...
    poly_fit <- lm(response ~ poly(predictor, degree = poly_degree, raw = TRUE))
  ...
  p <- ggplot(data, aes_string(x = predictor_name, y = response_name)) +
    ...
    labs(
      title = "Podmienená stredná hodnota a kvantilová funkcia",
  ...
  # Kvantilove krivky
  for (q in quantiles) {
    p <- p + geom_line(data = plot_data, aes_string(x = "X", y = paste0("Quantile_", q)),
                       size = 1, linetype = "dashed", color = "red")
  }

  # Bod pre specific_x
  p <- p + geom_point(aes(x = specific_x, y = specific_mean), color = "blue", size = 3)

  for (q in quantiles) {
    p <- p + geom_point(aes(x = specific_x, y = specific_quantiles[[as.character(q)]]), color = "red", size = 3)
  }

  # Popisky pre specific_x
...
```{r, warning=FALSE}
result <- model_conditional_mean_value(data, "mpg", "disp", 0.5, 100, "poly", 2)
...
```{r, warning=FALSE}
printVariables(data)
model_joint_distribution_density(data, c("hp", "carb"), "kernel")
```
...
### Vyber metody pre vypocet podmienenej stredne hodnoty E[Y|X]
- moznosti su: "loess" (Lokalna regresia, prisposobuje modely (linearne, kvadraticke) podla okolia), "gam" (Generalizačný aditívny model, scitava hladke funkcie), "spline" (podobné princípu MKP)
