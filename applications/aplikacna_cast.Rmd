...
mixture_joint_distribution <- function(data, discrete_vars, continuous_vars, model_type, bw) {
  ...
  # Modelovanie hustoty pouzitim jadroveho vyhladzovania
   if (model_type == "kernel") {
    ...
    # Funkcia na vypocet hustoty pre 3D
...
```{r, warning=FALSE}
model_joint_distribution_density <- function(data, selected_variables, model_type = NULL, use_copula = FALSE, copula_type = NULL, marginal_density = NULL, bw = NULL) {
...
plot_decision_boundary <- function(data, response_name, predictor_names, model, method) {
  ...
  p <- ggplot() +
    ...
    labs(
      title = paste0("Rozhodovacia hranica pre metódu: ", method),
...
```{r, warning=FALSE}
printVariables(data)
model_joint_distribution_density(data = mtcars, selected_variables = c("hp", "mpg"), model_type = "normal", use_copula = TRUE, copula_type = "Clayton", marginal_density = "log_dnorm")
```
...
predictor_names - nazvy premennych prediktorov, napr. c("hp", "cyl", ...)
