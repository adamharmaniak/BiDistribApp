...
model_mixture_joint_distribution <- function(data, selected_variables, model_type) {
  ...
  if (model_type == "normal") {
    ...
    fig_3d <- plot_ly(density_data, x = ~Weight, y = ~Age_Category, z = ~Density, 
                       ...
                       color = ~Age_Category, line = list(width = 4)) %>%
  ...
  # Modelovanie hustoty ako hustoty normalneho rozdelenia (dnorm) cez kopulu
  if (model_type == "copula") {
    data[[discrete_vars]] <- factor(data[[discrete_vars]])
    categories <- levels(data[[discrete_vars]])

    category_values <- as.numeric(as.factor(data[[discrete_vars]]))
    unique_categories <- sort(unique(category_values))

    weights <- table(data[[discrete_vars]]) / nrow(data)
    cor_val <- cor(data[[continuous_vars]], category_values, method = "spearman") # Spearman pre lepsiu robustnost
    
    copula_model <- normalCopula(param = cor_val, dim = 2, dispstr = "un")

    # Funkcia na vypocet zdruzenej hustoty pre kazdu kategoriu
    calculate_density_3D <- function(data, category) {
      sub_data <- filter(data, .data[[discrete_vars]] == category)
      if (nrow(sub_data) > 1) {
        mu_x <- mean(sub_data[[continuous_vars]])
        sigma_x <- sd(sub_data[[continuous_vars]])

        x <- seq(min(data[[continuous_vars]]), max(data[[continuous_vars]]), length.out = 100)
        u1 <- pnorm(x, mean = mu_x, sd = sigma_x)

        y_val <- as.numeric(as.factor(category))
        u2 <- ecdf(category_values)(y_val)

        copula_density <- dCopula(cbind(u1, rep(u2, length(u1))), copula = copula_model)
        x_density <- dnorm(x, mean = mu_x, sd = sigma_x)

        joint_density <- copula_density * x_density

        return(data.frame(Weight = x, Density = joint_density, Category = category))
      } else {
        return(data.frame(Weight = numeric(0), Density = numeric(0), Category = character(0)))
      }
    }
    ...
    # Vizualizacia v 3D s ciarami pre kazdu kategoriu
    fig_3d <- plot_ly(density_data, x = ~Weight, y = ~Category, z = ~Density, 
                       type = 'scatter3d', mode = 'lines', 
                       color = ~Category, line = list(width = 4)) %>%
              layout(scene = list(xaxis = list(title = continuous_vars),
                                  yaxis = list(title = discrete_vars),
                                  zaxis = list(title = 'Hustota')))
  ...
  # Modelovanie 3D hustoty ako hustoty normalneho rozdelenia (dnorm)
  if (model_type == "normal") {
    # 3D modelovanie
...
```{r,warning=FALSE}
selected_variables <- c("carb", "hp")
result <- model_mixture_joint_distribution(mtcars, selected_variables, "kernel")
print(result)
```
