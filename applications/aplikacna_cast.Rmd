...
```{r}
plot_conditional_densities <- function(data, selected_variables, conditional_density_type = "kernel", mean_curve = TRUE, quantiles = NULL, n_breaks = 5, density_scaling = 2000, normal_density = TRUE, empirical_density = TRUE) {
  ...
  if (!is.null(quantiles)) {
    for (q in quantiles) {
      ...
      p <- p + geom_line(data = quantile_df, aes(x = predictor, y = quantile_pred), color = "purple", linetype = "dashed") +
        guides(color = guide_legend(title = "Hustota (typ)"))
    }
  }
...
```{r}
printVariables(iris)
plot_conditional_densities(data = iris, selected_variables = c("Sepal.Length", "Petal.Length"), conditional_density_type = "kernel", mean_curve = TRUE, quantiles = c(0.1, 0.9), n_breaks = 4, density_scaling = 1, normal_density = TRUE, empirical_density = TRUE)
```
