...
combine_conditional_models <- function(data, selected_variables, mean_method = "linear", quantile_method = "linear", poly_mean_degree = NULL, poly_quant_degree = NULL, quantiles = 0.5, specific_x = NULL) {
  ...
  if (!(response_name %in% c(variable_types$Spojite, variable_types$Diskretne))) {
    stop(paste("Premenna", response_name, "nie je rozpoznana ako spojita alebo diskretna."))
  }
...
result <- combine_conditional_models(data, c("cyl", "mpg"), "poly", "poly", 3, 3, c(0.05, 0.95))
...
classification_model <- function(data, response_name, predictor_names, method = "logistic", k = NULL) {
  ...
  if (!(response_name %in% colnames(data))) {
    stop(paste("Premenná", response_name, "nie je v dátach!"))
  }
...
```{r}
result <- classification_model(data = mtcars, response_name = "carb", predictor_names = c("hp", "mpg"), method = "logistic")
...
*Pridat*
- pridat funkciu na modelovanie klasifikacnych metod (napr. logisticka regresia)
