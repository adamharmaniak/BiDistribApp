...
```{r, warning=FALSE}
continuous_joint_distribution_copula <- function(data, continuous_vars, model_type, copula_type, marginal_density = "dnorm") {
  ...
  # Funkcia na vypocet marginalnej hustoty podla vyberu
    marginal_density_function <- function(value, mean, sd) {
      if (marginal_density == "dnorm") {
        return(dnorm(value, mean = mean, sd = sd))
      } else if (marginal_density == "log_dnorm") {
  ...
  copula_density_function <- function(x, y) {
    ...
    marginal_x <- marginal_density_function(x, mean_x, sd_x)
    marginal_y <- marginal_density_function(y, mean_y, sd_y)
...
```{r, warning=FALSE}
model_joint_distribution_density <- function(data, selected_variables, model_type = NULL, bw = NULL, use_copula = FALSE, copula_type = NULL, marginal_density = NULL) {
  ...
  if (length(selected_variables) == 2 && length(continuous_vars) == 0) {
    ...
    if (use_copula == FALSE) {
      ...
      if (is.null(copula_type)) {
        ...
        if (model_type != "normal" && !is.null(marginal_density)){
          stop("Parameter marginal_density sa zadava iba pri model_type = 'normal' s kopulou")
        }
...
# Modelovanie podmienenej strednej hodnoty a vybraných kvantilových funkcií (regresia)
...
```{r}
result <- combine_conditional_models(data, c("carb", "hp"), "spline", "poly", NULL, 2, c(0.05, 0.95))

result$combined_plot
```

# Modelovanie predikčných modelov pre diskrétne premenné (klasifikačné metódy)
```{r}
...
plot_classification_1D <- function(data, response_name, predictor_name, model, method) {

    if (!is.factor(data[[response_name]])) {
      data[[response_name]] <- as.factor(data[[response_name]])
    }
  
    grid_x <- seq(min(data[[predictor_name]]), max(data[[predictor_name]]), length.out = 300)
    grid <- data.frame(x = grid_x)
    names(grid) <- predictor_name
    ...
    # Vytvorenie grafu
    p <- ggplot() +
      geom_tile(data = grid, aes_string(x = predictor_name, y = "pred", fill = "pred"), alpha = 0.3, height = 0.5) +
      
      # Skutočné hodnoty (triedy)
      geom_point(data = data,
                 aes_string(x = predictor_name, y = response_name, shape = response_name, color = response_name),
                 size = 3) +
  
      labs(
        title = paste0("Predikčný model metódy: ", method),
        subtitle = paste("Prediktor:", predictor_name, "| Odozva:", response_name),
        x = paste0(predictor_name, " (Prediktor)"),
        y = paste0(response_name, " (Trieda)"),
        fill = "Predikcia modelu"
      ) +
...
classification_model <- function(data, response_name, predictor_names, method = "logistic", k = NULL) {
  ...
  if (method == "logistic") { # logisticka regresia
    ...
    train_data <- data[, predictor_names]
  ...
  confusion_mat <- table(Predicted = preds, Actual = response)
  ...
  if (length(predictor_names) == 1) {
    ...
    result$decision_plot <- plot_classification_1D(
      data, response_name, predictor_names[1], model, method
    )

  } else if (length(predictor_names) == 2) {
    # Ak su len 2 prediktory => graf rozhodovacich hranic
...
```{r}
result <- classification_model(data = mtcars, response_name = "carb", predictor_names = c("mpg", "hp"), method = "logistic")
...
# Finálne výstupy
```{r, warning=FALSE}
printVariables(mtcars)
model_joint_distribution_density(data = mtcars, selected_variables = c("mpg", "carb"), model_type = "normal")
```

```{r}
plot_conditional_densities <- function(data, selected_variables, conditional_density_type = "kernel", mean_curve = TRUE, quantiles = NULL, n_breaks = 5, density_scaling = 2000) {
  
  if (length(selected_variables) != 2) {
    stop("Musis zadat dve premenne: odozvu a prediktor.")
  }
  ...
  density_data <- do.call(rbind, lapply(split(df, df$section), function(sub_df) {
    ...
    # Empiricka hustota
    emp_density <- density(sub_df$residuals, n = 50)
    
    # Hustota normalneho rozdelenia
    xs <- seq(min(sub_df$residuals), max(sub_df$residuals), length.out = 50)
    theoretical_density <- dnorm(xs, mean = 0, sd = sd(sub_df$residuals, na.rm = TRUE))
    ...
    df_theo <- data.frame(
      x = x_max - theoretical_density * density_scaling,
      y = xs + y_mean,
      section = unique(sub_df$section),
      type = "normal"
    )
    
    rbind(df_emp, df_theo)
  }))
  ...
  p <- ggplot(df, aes(x = predictor, y = response)) +
    ...
    theme_bw() +
    labs(
      title = paste("Podmienené hustoty pre", response_var, "podľa", predictor_var),
      x = paste(predictor_var, "(Prediktor)"),
      y = paste(response_var, "(Odozva)")
    ) +
  ...
  if (mean_curve) {
    p <- p + geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "blue", size = 1.2)
  }
  
  # Kvantilove krivky
  if (!is.null(quantiles)) {
    for (q in quantiles) {
      rq_fit <- quantreg::rq(response ~ predictor, tau = q, data = df)
      quantile_df <- df %>% mutate(quantile_pred = predict(rq_fit, newdata = df))
      
      p <- p + geom_line(data = quantile_df, aes(x = predictor, y = quantile_pred), color = "purple", linetype = "dashed") +
        guides(color = guide_legend(title = "Hustota / Kvantil"))
    }
  }
...
```{r}
plot_conditional_densities(data = iris, selected_variables = c("Sepal.Length", "Sepal.Width"), conditional_density_type = "kernel", mean_curve = TRUE, quantiles = c(0.1, 0.9), n_breaks = 3, density_scaling = 1)
```
...
Vyriesene:
  ...
  - normal_copula - dat uzivatelovi moznost vyberu pocitania marginalnych hustot (napr. cez dnorm() alebo lognorm atd.)
  - rozdelit funkciu model_conditional_mean_value na dve
  - pridat funkciu na modelovanie klasikacnych metod (napr. logisticka regresia)
  - vyriesit normovanie plochy pod hustotou pre mix premennych na sucet = 1
  - modelovanie podmienenej hustoty pravdepodobnosti (zatial iba pre jadrove vyhladzovanie)
Riesim:
  - modelovanie podmienenej pravdepodobnostnej funkcie (v pripade diskretnej premnnej ako odozvy)
