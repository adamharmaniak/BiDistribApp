...
plot_classification_1D_combined <- function(data, response_name, predictor_name, model, method) {
  ...
  # Typ prediktora
  if (is.numeric(data[[predictor_name]]) && length(unique(data[[predictor_name]])) > 5) {
  ...
  if (predictor_type == "continuous") {
    grid_x <- seq(
      min(data[[predictor_name]], na.rm = TRUE),
      max(data[[predictor_name]], na.rm = TRUE),
      length.out = 300
    )
  } else {
    grid_x <- sort(unique(data[[predictor_name]]))
  }

  grid <- setNames(data.frame(grid_x), predictor_name)

  # Ziskanie predikcii / pravdepodobnosti podla metody
  probs_long <- NULL
  ...
  if (method == "logistic" && length(classes) > 2) {
    ...
    probs_long <- probs %>%
      tidyr::pivot_longer(
        cols = all_of(classes),
        names_to = "Hodnota_odozvy",
        values_to = "Pravdepodobnost"
      )
    ...
    grid$pred_class <- ifelse(p > 0.5, classes[2], classes[1]) %>%
      factor(levels = classes)
  ...
  # Pravdepodobnosti podla hodnot odozvy
  p1 <- NULL
  if (!is.null(probs_long)) {
    ...
    if (predictor_type == "continuous") {
      ...
      p1 <- ggplot(probs_long, aes(x = .data[[predictor_name]], y = Pravdepodobnost, color = Hodnota_odozvy)) +
        ...
        scale_color_manual(values = my_colors, name = "Hodnota_odozvy") +
        labs(
      ...
      p1 <- ggplot(probs_long, aes(x = factor(.data[[predictor_name]]), y = Pravdepodobnost, fill = Hodnota_odozvy)) +
        ...
        scale_fill_manual(values = my_colors, name = "Hodnota_odozvy") +
        labs(
          title = paste0("Pravdepodobnosti predikcie - ", method),
          x = predictor_name,
          y = "Pravdepodobnost"
  ...
  # Skutocne hodnoty odozvy + rozhodovacia hranica
  p2 <- ggplot() +
    geom_tile(
      data = grid,
    ...
    geom_jitter(
      data = data,
      aes(x = .data[[predictor_name]], y = .data[[response_name]], color = .data[[response_name]], shape = .data[[response_name]]),
      width = 0.1,
      height = 0.1,
      size = 2
    ) +
    scale_fill_manual(values = my_colors, name = "Predikcia") +
    scale_color_manual(values = my_colors, name = "Skutocna hodnota odozvy") +
    scale_shape_manual(values = my_shapes, name = "Skutocna hodnota odozvy") +
    labs(
...
```{r}
plot_conditional_continuous_densities <- function(df, n_breaks = 5, density_scaling = 2000, mean_curve = TRUE, quantiles = NULL, mean_poly_degree = 1, quantile_poly_degree = 1, normal_density = TRUE, empirical_density = TRUE, bw = NULL) {
  ...
  density_data <- do.call(rbind, lapply(split(df, df$section), function(sub_df) {
    ...
    if (empirical_density) {
      if (is.null(bw)){
        ...
        emp_density <- density(sub_df$residuals, n = 50, bw = bw)
      }
...
plot_conditional_densities <- function(data, selected_variables, n_breaks = 5, density_scaling = 2000, ordinal = FALSE, mean_curve = TRUE, quantiles = NULL, mean_poly_degree = 1, quantile_poly_degree = 1, normal_density = TRUE, empirical_density = TRUE, bw = NULL) {
  ...
  if (response_var %in% var_types$Diskretne) {
    ...
    plot_conditional_continuous_densities(df, n_breaks, density_scaling, mean_curve, quantiles, mean_poly_degree, quantile_poly_degree, normal_density, empirical_density, bw)
...
model_joint_distribution_density(data = mtcars, selected_variables = c("hp", "mpg"), model_type = "t", bw = NULL, use_copula = TRUE, copula_type = "Joe", marginal_density = NULL)
```

```{r}
printVariables(mtcars)
result <- classification_model(data = mtcars, response_name = "carb", predictor_names = c("hp"), method = "knn", k = 5)
...
```{r}
printVariables(mtcars)
plot_conditional_densities(data = mtcars, selected_variables = c("carb", "hp"), n_breaks = 5, density_scaling = 50, ordinal = TRUE, mean_curve = FALSE, quantiles = NULL, mean_poly_degree = 2, quantile_poly_degree = 3, normal_density = FALSE, empirical_density = TRUE, bw = 1.2)
```
...
Vyriesene:
  *plot_conditional_densities*
    - spajat body podm. pravd. fun. pri ordinalnych disk. prem.
    - pridat na vstup mieru zhladzovania
  - vrstevnice pre modelovanie hustoty dvoch spojitych premennych
  - explicitne klasifikacny model pre jeden prediktor
