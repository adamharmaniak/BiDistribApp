...
\begin{figure}[H]
    ...
    \caption{Podmienená stredná hodnota pre premenné $Y=$ \textit{Dĺžka zobáka tučniaka} a $X=$ \textit{Hmotnosť tučniaka v gramoch} s vypočítanou hodnotou v $\mathbb{E}[Y \mid X=5000]$}
    \label{fig:cond_mean}
\end{figure}
...
Parametre $\beta_\tau$ sa odhadujú pomocou minimalizácie cieľovej funkcie založenej na tzv. \textit{"check function"}:
...
\begin{itemize}
  ...
  \item Je robustná voči odľahlým hodnotám, obzvlášť pri odhadoch mediánu ($q = 0{,}5$).
\end{itemize}
